...
export class BackendSyncService {
  ...
  async pullPeriods(): Promise<IFocus.Period[]> {
    ...
    const response = await firstValueFrom(
      this.#http.get<{ success: boolean; data: IFocus.Period[] }>(`${this.#apiUrl}/periods`, {
        headers,
      })
    );
  ...
  async pushPeriod(period: IFocus.Period): Promise<void> {
    ...
    await firstValueFrom(this.#http.post(`${this.#apiUrl}/periods`, period, { headers }));
  }
  ...
  async checkHealth(): Promise<boolean> {
    try {
      const response = await firstValueFrom(
        this.#http.get<{ success: boolean }>(`${this.#apiUrl}/health`)
      );
